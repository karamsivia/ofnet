...
const url_links = "http://" + bmp_agent + ":" + bmp_port + "/db_rest/v1/linkstate/links/epe"
...
func find_router_name(router_data tbl_router, router_ip string) string {
...
func find_next_hop(lookup_data tbl_lookup, router, sla string) string {
	...
	for idx := 0; idx < lookup_data.VAllRoutes.Size; idx++ {
		...
		if has_the_sla(lookup_data.VAllRoutes.Data[idx].ExtCommunities, sla) &&
			lookup_data.VAllRoutes.Data[idx].RouterName == router &&
	...
	log.Debugf(" In: find_next_hop:.......shortest AS: %d\n", shortest_AS)
	return peer_addr
		...
		if (link_state == "Active") && (remote_ip == peer_addr) {
			...
			*/
			...
			if len(peer_node_SID) == 0 {
				...
				log.Debugf("Error: The peer node SID label is empty :%s for remoteIP: %s", peer_node_SID, peer_addr)
				return link

			}

			//log.Debugf("%q",peer_node_SID)
...
func Epe_label_SR(source, target, ver_sla string) (bool, []string) {
	...
	for idx := 0; idx < lookup_data.VAllRoutes.Size; idx++ {
		...
		*/
		if lookup_data.VAllRoutes.Data[idx].PeerAddress == "0.0.0.0" {
			next_hop_name = lookup_data.VAllRoutes.Data[idx].PeerName
			log.Debugf(".......................Next Hop Name:%s\n", next_hop_name)
			break
		}
	}
	...
	*/
	...
	for i := 1; i < 4; i++ {
		peer_addr = ""
		peer_addr = find_next_hop(lookup_data, next_hop_name, sla)
		if peer_addr == "" {
		...
		log.Debugf(" ........................Label AS: %s\n", epe_label)
		label_stack = append(label_stack,epe_label)
		next_hop_name = find_router_name(router_data, next_hop_ip)
		log.Debugf(".......................Peer Addr : %s AND BGP Id:%s Name:%s\n", peer_addr, next_hop_ip, next_hop_name)
