...
// Mpls state.
// One Mpls instance exists on each host
type Mpls struct {
...
func NewMpls(agent *OfnetAgent, rpcServ *rpc.Server) *Mpls {
	mpls := new(Mpls)
	...
	// Create policy agent
	mpls.policyAgent = NewPolicyAgent(agent, rpcServ)

	// Create a flow dbs and my router mac
	mpls.flowDb = make(map[string]*ofctrl.Flow)
	mpls.portVlanFlowDb = make(map[uint32]*ofctrl.Flow)
	mpls.myRouterMac, _ = net.ParseMAC("00:00:11:11:11:11")
	mpls.unresolvedEPs = make(map[string]string)

	return mpls
}

// Handle new master added event
func (self *Mpls) MasterAdded(master *OfnetNode) error {

	return nil
}

// Handle switch connected notification
func (self *Mpls) SwitchConnected(sw *ofctrl.OFSwitch) {
	// Keep a reference to the switch
	self.ofSwitch = sw

	log.Infof("Switch connected(mpls). installing flows")
...
func (self *Mpls) SwitchDisconnected(sw *ofctrl.OFSwitch) {
	// FIXME
}

// Handle incoming packet
func (self *Mpls) PacketRcvd(sw *ofctrl.OFSwitch, pkt *ofctrl.PacketIn) {
...
func (self *Mpls) AddLocalEndpoint(endpoint OfnetEndpoint) error {
...
*/
func (self *Mpls) RemoveLocalEndpoint(endpoint OfnetEndpoint) error {
...
// Add IPv6 flows
func (self *Mpls) AddLocalIpv6Flow(endpoint OfnetEndpoint) error {
...
func (self *Mpls) RemoveLocalIpv6Flow(endpoint OfnetEndpoint) error {
...
func (self *Mpls) AddVlan(vlanId uint16, vni uint32, vrf string) error {
	log.Infof("ADD VLAN %v" , vrf)	
	vrf = "default"
...
// Remove a vlan
func (self *Mpls) RemoveVlan(vlanId uint16, vni uint32, vrf string) error {
...
*/
func (self *Mpls) AddEndpoint(endpoint *OfnetEndpoint) error {
...
func (self *Mpls) RemoveEndpoint(endpoint *OfnetEndpoint) error {
...
func (self *Mpls) AddRemoteIpv6Flow(endpoint *OfnetEndpoint) error {
...
func (self *Mpls) RemoveRemoteIpv6Flow(endpoint *OfnetEndpoint) error {
...
func (self *Mpls) initFgraph() error {
	sw := self.ofSwitch
	...
	// Init policy tables
	err := self.policyAgent.InitTables(SRMPLS_TBL_ID)
	if err != nil {
	...
	srPktFlow, _ := self.srmplsTable.NewFlow(ofctrl.FlowMatch{
                Priority: FLOW_MISS_PRIORITY,
        })
        srPktFlow.Next(self.ipTable)
...
*/
func (self *Mpls) processArp(pkt protocol.Ethernet, inPort uint32) {
...
func (self *Mpls) AddVtepPort(portNo uint32, remoteIp net.IP) error {
	return nil
}

// Remove a VTEP port
func (self *Mpls) RemoveVtepPort(portNo uint32, remoteIp net.IP) error {
	return nil
}
...
func (self *Mpls) resolveUnresolvedEPs(MacAddrStr string, portNo uint32) {
...
func (self *Mpls) AddUplink(portNo uint32) error {
...
func (self *Mpls) RemoveUplink(portNo uint32) error {
	return nil
}

// AddSvcSpec adds a service spec to proxy
func (self *Mpls) AddSvcSpec(svcName string, spec *ServiceSpec) error {
	return nil
}

// DelSvcSpec removes a service spec from proxy
func (self *Mpls) DelSvcSpec(svcName string, spec *ServiceSpec) error {
	return nil
}

// SvcProviderUpdate Service Proxy Back End update
func (self *Mpls) SvcProviderUpdate(svcName string, providers []string) {
}
